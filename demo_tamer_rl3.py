...
def DEMO_TAMER_RL(
    ...
    def normal_decay(x, mean=0.5, std=0.2):
        power = np.exp(-((x-mean)**2/(2*std**2)))
        return np.tanh(power / (std * (2*np.pi) ** 0.5))
    ...
    def type6_action(s, done, w, w_h, w_D, h_alpha, epsilon=.0, frac=0):
        nA = env.action_space.n
        ...
        if np.random.rand() < normal_decay(frac):
    ...
    for i_epi in range(num_episode):
        ...
        while not done:
            ...
            a_dash = type6_action(s_dash, done, w, w_H, w_D, h_alpha, epsilon=demo_epsilon, frac=ep_len/200)
