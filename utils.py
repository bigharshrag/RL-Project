...
def greedy_policy(w, s, done, X, nA):
...
def _eval(env, w, X):
    ...
    G = 0.
    while not done:
        a = greedy_policy(w, s, done, X, env.action_space.n)
        s,r,done,_ = env.step(a)

        G += r
    return G

def evaluate(env, w, X):
    Gs = [_eval(env, w, X) for _ in  range(100)]
    return np.mean(Gs)
...
#     print(di)
#     avg_traj.append(len(dem))
#     for ps, pa, _ in dem:
#         state_feat.append(X(ps, 0))
#         state_feat_ac.append(pa)
# print("Avg len of demo: ", np.mean(avg_traj))
